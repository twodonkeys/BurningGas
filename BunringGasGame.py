...
class BuringGas():
    def __init__(self):#初始化
        ...
        self.Component_H2=0.105
        self.Component_CH4=0.01
        self.Component_C2H4=0.001
        self.Component_CO2=0.105
        self.Component_N2=0.615
        self.Component_O2=0.001
        self.Componnent_H2O=0.023
        #煤气参数
        self.Radius_Gas=1.2 #煤气管道半径m
        self.Density_Gas=1.363 #煤气密度，不能为0
        self.Pressure_Gas=10 #煤气压力
        self.Flow_K_Gas=0.65 #煤气流量系数（0.6~0.65）
        self.Flow_Gas=20000 #煤气流量
        self.ValueOpening_Gas=50 #煤气阀门开度
        self.T_pre_Gas=150#煤气预热温度
        # 空气参数
        self.Radius_Air=0.7#空气管道半径
        self.Density_Air=1.293 #空气密度,不能为0
        self.Pressure_Air=9 #空气压力
        self.Flow_K_Air=0.65 #空气流量系数（0.6~0.65）
        self.Flow_Air=20000 #空气流量
        self.ValueOpening_Air=50 #空气阀门开度
        self.T_pre_Air#空气预热温度
    def Gas_Heat(self,CO,H2,CH4,C2H4):#热值
        return (127.7*CO+107.6*H2+358.8*CH4+599.6*C2H4)*100

    def AirFuelRatio(self,CO,H2,CH4,C2H4,O2):#理论最优空燃比，单位煤气所用空气量
    ...
    def V_burned(self,H2,CO,CH4,C2H4,CO2,N2,H2O,AirFuelRatio):#单位（m3）煤气燃烧后的气体体积
        return (H2 + CO + 3 * CH4 + 4 * C2H4 + CO2 + N2 + H2O) + 0.79 * AirFuelRatio

    def FirstOrderInertia(self,input,output,lamda):#lamda越小延迟越大
        lamda=0 if (1 if lamda>1 else lamda)<0 else lamda
        output=input*lamda+output(1-lamda)
        output=input if abs(output-input)<1 else output
        return output
    ...
    def buring(self):
        if self.Flow_Gas>(self.Flow_Air/self.AirFuelRatio):#煤气过量
            ...
            self.V_burned_H2O=abs((self.Component_H2+2*self.Component_CH4)+2*self.Component_C2H4)\
                         *(self.AirFuelRatio_Actual/self.AirFuelRatio)+self.Componnent_H2O/self.V_burned
            ...
            self.V_burned_N2=abs((self.Component_N2+0.79*self.AirFuelRatio)/self.V_burned)
    ...
    def reset(self):
        time.sleep(0.5)
        self.__init__()
        self.step(self.ValueOpening_Gas,self.ValueOpening_Air)

    def print_out(self):
        print("\r******燃烧仿真******")

    def step(self, valve_Gas, valve_Air):  # 动作传入
        self.ValueOpening_Gas=self.FirstOrderInertia(valve_Gas,self.ValueOpening_Gas,0.5)
        self.ValueOpening_Air=self.FirstOrderInertia(valve_Air,self.ValueOpening_Air,0.5)

        # 煤气流量
        self.Flow_Gas = self.Flow(Radius=self.Radius_Gas, Density=self.Density_Gas, Pressure=self.Pressure_Gas,
                                  Flow_K=self.Flow_K_Gas, ValueOpening=self.ValueOpening_Gas)
        # 空气流量
        self.Flow_Air = self.Flow(Radius=self.Radius_Air, Density=self.Density_Air, Pressure=self.Pressure_Air,
                                  Flow_K=self.Flow_K_Air, ValueOpening=self.ValueOpening_Air)
        # 单位煤气最佳燃烧所需空气量：理论最佳空煤比
        self.AirFuelRatio = self.AirFuelRatio(CO=self.Component_CO, H2=self.Component_H2, CH4=self.Component_CH4,
                                              C2H4=self.Component_C2H4, O2=self.Component_O2)
        # 单位煤气理论生成废气
        self.V_burned_Unit = self.V_burned(H2=self.Component_H2, CO=self.Component_CO, CH4=self.Component_CH4,
                                           C2H4=self.Component_C2H4, CO2=self.Component_CO2, N2=self.Component_N2,
                                           H2O=self.Componnent_H2O, AirFuelRatio=AirFuelRatio)
        # 单位煤气热量：热值
        ...
        self.C_Gas = 1.2979 * self.Component_CO + 1.277 * self.Component_H2 + 1.5491 * self.Component_CH4 \
                     ...
                     + 1.3063 * self.Component_O2 + 1.4947 * self.Componnent_H2O + 0.0075 * self.T_pre_Gas / 100
        # 废气比热容
        self.C_Exhaust = 1.2979 * self.V_burned_CO + 1.277 * self.V_burned_H2 + 1.5491 * self.V_burned_CH4 \
                         ...
                         + 1.3063 * self.self.V_burned_O2 + 1.4947 * self.V_burned_H2O
        # 单位煤气燃烧释放的热量
        ...
        # 理论燃烧温度计算
        self.T_buring = self.i_Sum / self.C_Exhaust
